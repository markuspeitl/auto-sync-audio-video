...
def detect_audio_activity_trim(audio_src_path: str, args: argparse.Namespace):
    ...
    if (not args.active_range_timestamps):
        args.active_range_timestamps = detect_audio_activity_range_of_file(audio_src_path, args)

    if (not args.no_file_trimming):

        trimmed_file_path = add_name_suffix_path(audio_src_path, f"_trimmmed_{args.active_range_timestamps[0]}-{args.active_range_timestamps[1]}")
        trimmed_file_path = trim_media_output(audio_src_path, trimmed_file_path, args.active_range_timestamps, overwrite_target=args.overwrite_target)

        if (args.trim_src_inplace and move_overwrite(trimmed_file_path,  audio_src_path)):
            trimmed_file_path = audio_src_path

        print(trimmed_file_path)
        return trimmed_file_path

    return args.active_range_timestamps
